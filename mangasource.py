...
from webpage import Webpage, link

from models import Manga, session
...
class MangaSource(object):
    ...
    def __init__(self):
        ...
        self.query = Manga.query.filter_by(mangasite=cls_name)
    ...
    def get_manga_list_pages(self, **kwargs):
        """fetches and returns the list url as a Webpage object"""
        list_urls = self.get_manga_list_urls()
    ...
    def get_manga_list(self):
        """returns a list of manga as namedtuples"""
        pages = self.get_manga_list_pages()
        #single page, default case
        if len(pages) == 1:
            manga_list = self.parse_manga_list(pages[0])
        #multiple pages, do it async
        else:
            manga_list = []
            for p in pages:
                manga_list.extend(self.parse_manga_list(p))
        return self._get_slice(manga_list, "manga_list")
    ...
    def update_list(self):
        ...
        manga_list = [{'name': m.text, 'link': m.url, 'last_updated': None,
                        'mangasite': self.__class__.__name__}
                        for m in manga_list]
        ...
        manga_list = [Manga(**m) for m in manga_list]
